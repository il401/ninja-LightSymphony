  ...
  */
  ...
  if (typeof functionToCall !== 'undefined') {
    if (turnOnFirst) {
	  ...
	  //default action?? only used for LL colours I think
	  // might need this if activating dimmer?
    }
    else {
      functionToCall();
    }
...
function sendUDPascii(cmd, ipAddress, port, callback) {
    ...
    sendUDPCommand(message, valByteNA, ipAddress, port, callback);
	Console.log('ran: sendUDPCommand(' & message & ',' & valByteNA & ',' & ipAddress & ',' & port  & ',' & callback & ')');
}
...
function sendUDPCommand(cmdByte, valByte, ipAddress, port, callback) {
  ...
  client.send(cmd, 0, cmd.length, port, ipAddress, function(err, bytes) {
      if (err) throw err;
      console.log('UDP message [' & cmdByte & '] sent to ' + HOST +':'+ PORT);
      //count++;	
  });
